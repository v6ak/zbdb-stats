...
object Parser{
  ...
  private def parseTimeInfo(data: Seq[String], prevTimeOption: Option[Moment], maxHourDelta: Int): Option[PartTimeInfo] = data match {
  ...
  def parse(csvData: String, startTime: Moment, totalEndTime: Moment, maxHourDelta: Int) = {
    val fullDataTable = new CSVReader(new StringReader(csvData)).toIndexedSeq.map(_.toIndexedSeq)
    val Seq(title, header1, header2, header3, dataWithTail @ _*) = fullDataTable
    val dataTable = dataWithTail.dropRight(2)
    val Seq(footer1, footer2) = dataWithTail.takeRight(2)
    assertEmpty(footer1.take(5).toSet -- Set(""))
    assertEmpty(footer2.toSet -- Set("", "nejdříve na stanovišti", "nejrychleji projitý úsek"))
    ...
    val parsedDataTries = dataTable.map{participantData =>
      Try(
        Left(parseParticipant(participantData, parts, startTime, maxHourDelta, totalEndTime = totalEndTime))
      ).recover{
    ...
    (parts, parsedDataSuccessfulTries.map{case Left(p) => p})
  }

  private def parseParticipant(participantData: immutable.IndexedSeq[String], parts: immutable.IndexedSeq[Part], startTime: Moment, maxHourDelta: Int, totalEndTime: Moment): Participant = {
